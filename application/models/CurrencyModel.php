...
class CurrencyModel extends CI_Model {
	...
	public function getCurrency($userID) {
		$this->db->select('user_id , currency');
	...
	public function updateCurrency($userID, $value) {
		$this->db->set(array('currency' => $value));
		$this->db->where(array('user_id' => $userID));
		$q = $this->db->update('tbl_users');


		$this->db->where(array('user_id' =>$userID));
		$insertedRow = $this->db->get('tbl_users');

		return $insertedRow->row_array();
	}

	public function getLatestCurrencyLog($userId) {

		$this->db->limit(1);
		$this->db->order_by('entry_date', 'DESC');
		$q = $this->db->get_where('tbl_currency_log', array('user_id' => $userId));
		
		if($q->num_rows() ==0) {

			// do an insert
			$newCurrencyLog = array(
				'user_id' => $userId,
				'starting_balance' => 50,
			...
			$insert = $this->db->insert('tbl_currency_log', $newCurrencyLog);

			$id = $this->db->insert_id();
			$q = $this->db->get_where('tbl_currency_log', array('entry_id' => $id));
	...
	public function addDepositLog($userId, $betId, $betAmount, $previousBalance, $source) {

		$newCurrencyLog = array(
			'user_id' => $userId,
			'starting_balance' => $previousBalance,
			'transaction_amount' => $betAmount,
			'ending_balance' => $previousBalance + $betAmount,
	...
	public function addWithdrawLog($userId, $betId, $betAmount, $previousBalance) {

		return $this->addDepositLog($userId, $betId, $betAmount * (-1), $previousBalance, 2);
	}

	public function getAllWagerEntries($betID) {
		// echo 'in the refundWagers';
		$this->db->where(array('bet_id' => $betID));
		$q = $this->db->get('tbl_currency_log');
		if($q->num_rows()>0) {
			$q = $q->result_array();
			return $q;
		}
